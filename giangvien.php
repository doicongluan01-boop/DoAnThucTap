...
<head>
    ...
    <style>
        ...
        .btn-delete:hover {
            ...
            background: linear-gradient(135deg, #ff4757, #c31432);
        }
        .badge-sv {
            font-size: 1rem;
            padding: 10px 18px;
        ...
        .toast .progress { height: 5px; background: rgba(255,255,255,0.3); }
        .toast .progress-bar { background: white; animation: progress 4s linear forwards; }
        @keyframes slideIn { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
        @keyframes progress { from { width: 100%; } to { width: 0%; } }
        @media (max-width: 768px) {
            .page-header h3 { font-size: 1.4rem; }
            .btn-import { width: 100%; margin: 10px 0; }
        }
    </style>
</head>
<body>
...
<div class="container-fluid">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle">
            ...
            <tbody>
                <?php
                // LẤY SỐ SINH VIÊN ĐANG HƯỚNG DẪN CHUẨN (DỪNG BẢNG NHÓM MỚI)
                $stmt = $pdo->query("
                    SELECT 
                        g.MaGV,
                        g.HoTen,
...
document.getElementById('formImportGV')?.addEventListener('submit', function(e) {
    ...
    fetch('import_gvhd_real.php', { method: 'POST', body: fd })
        ...
        .catch(() => {
            showToast('toast-error', 'Lỗi kết nối server!');
        })
        .finally(() => {
            btn.disabled = false;
