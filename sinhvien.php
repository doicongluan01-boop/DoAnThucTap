...
// 2. TRUY VẤN DỮ LIỆU (ĐÃ TỐI ƯU HÓA)
// Thay đổi: Thêm JOIN bảng giang_vien ngay tại đây để lấy tên GVHD luôn
// Không cần query lại trong vòng lặp nữa -> Tốc độ tăng gấp 10 lần

if ($isGiangVien) {
    // Logic: Xem SV mình hướng dẫn HOẶC SV trong nhóm mình phản biện
    $sql = "
        SELECT DISTINCT
            s.*,
            n.id AS nhom_id,
            n.ten_nhom AS nhom,
            gv.HoTen AS gvhd_ten  -- Lấy luôn tên GVHD
        FROM danh_sach_sinh_vien s
        INNER JOIN phan_thuoc_nhom ptn ON ptn.sinh_vien_id = s.id
        INNER JOIN nhom n ON n.id = ptn.nhom_id
        LEFT JOIN giang_vien gv ON n.giang_vien_huong_dan_id = gv.MaGV -- Join lấy tên
        WHERE
...
<head>
    ...
    <style>
        ...
        /* Giữ lại style cho Stat Cards nhưng nhẹ hơn */
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#mainTable').DataTable({
            ...
            pageLength: 25, // Mặc định hiện 25 dòng cho nhẹ máy
            ordering: true,
            responsive: true,
            columnDefs: [
                { orderable: false, targets: -1 } // Không sắp xếp cột tác vụ
            ]
        });

        // XỬ LÝ XÓA (Logic cũ của bạn nhưng tích hợp vào bảng mới)
        $(document).on('click', '.delete-real', function() {
            const btn = $(this);
            const mssv = btn.data('mssv');
            // Lấy tên từ cột thứ 3 (index 2) trong dòng hiện tại
            ...
            fetch('xoa_sinhvien.php', { method: 'POST', body: fd })
                ...
                .then(res => {
                    if (res.success) {
                        // Xóa dòng khỏi DataTables để không lỗi phân trang
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                ...
                <div class="row">
                    ...
                    <img src="/image/mau_import.png" alt="50">
                </div>
