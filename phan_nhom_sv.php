...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'gan_nhom') {
    ...
    if (empty($sinhvien_ids) || empty($ten_nhom)) {
        echo json_encode(['success' => false, 'msg' => 'Chưa chọn sinh viên hoặc tên nhóm!']);
        exit;
    }
    ...
    try {
        ...
        echo json_encode([
            'success' => true,
            'msg' => "Đã gán thành công " . count($sinhvien_ids) . " sinh viên vào <strong>$ten_nhom</strong>!"
        ]);
    } catch (Exception $e) {
        $pdo->rollBack();
                ...
                <div class="col-lg-4">
                    <div class="bg-white rounded-3 shadow p-4" style="border: 2px dashed #667eea;">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="ten-nhom" class="form-control form-control-lg text-center" 
                                   placeholder="VD: Nhóm 1, Nhóm A, Nhóm 2025..." required>
                        </div>
...
function ganNhom() {
    ...
    Swal.fire({
        title: 'Xác nhận?',
        text: `Gán ${checked.length} sinh viên vào nhóm: "${tenNhom}"?`,
        icon: 'question',
