...
document.addEventListener("DOMContentLoaded", () => {
    // ===== LOGIN FORM – CHỈ CHẠY NẾU CÓ FORM LOGIN =====
        ...
        loginForm.onsubmit = async (e) => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (result.role === "giangvien") localStorage.setItem("MaGV", result.username);
                    if (result.role === "sinhvien") localStorage.setItem("MaSV", result.username);
    ...
    // ===== UPLOAD BIÊN BẢN – CHỈ CHẠY NẾU CÓ NÚT NỘP =====
    const nopBB = document.getElementById("nopBienBan");
    if (nopBB) {
        nopBB.onclick = () => uploadBB();
    }

    // ===== LOAD DANH SÁCH BIÊN BẢN – CHỈ CHẠY NẾU CÓ BẢNG =====
    const listBB = document.getElementById("listBB");
    if (listBB) {
        loadBB();
    }

    // ===== CÁC HÀM KHÁC (submenu, nhóm, v.v.) – cũng thêm kiểm tra null tương tự nếu cần
});
//====================================================
...
function showSubmenu(clickedElement) {
  ...
  if (main === "trangchu") {
    ...
    html = `
     <a href="#" onclick="loadFile(this, 'giangvien.php')">Danh sách giáo viên hướng dẫn</a>
  ...
  //   `;
  // }
  else if (main === "baocao") {                     // THÊM MỚI MENU "BÁO CÁO"
    html = `
...
async function importSmart(e) {
    if (input.files && input.files[0]) {
        ...
        // 3. Gọi file xử lý PHP (Logic này giữ nguyên như code cũ của bạn)
        // Lưu ý: Đảm bảo bạn đã sửa ID input trong code JS cũ thành 'real_import_input' 
        // hoặc dùng tham số 'input' được truyền vào hàm này là chuẩn nhất.
